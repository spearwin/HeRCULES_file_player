...
ROSThread::~ROSThread() {
  ...
  if (continental_thread_.thread_.joinable())
    continental_thread_.thread_.join();

  continentalobject_thread_.cv_.notify_all();
  if (continentalobject_thread_.thread_.joinable())
    continentalobject_thread_.thread_.join();
  ...
  if (stereo_right_thread_.thread_.joinable())
    stereo_right_thread_.thread_.join();

  stereo_left_thread_.cv_.notify_all();
  if (stereo_left_thread_.thread_.joinable())
    stereo_left_thread_.thread_.join();
}
...
void ROSThread::ros_initialize(ros::NodeHandle &n) {
  ...
  timer_ = nh_.createTimer(ros::Duration(0.0001),
                           boost::bind(&ROSThread::TimerCallback, this, _1));

  start_sub_ = nh_.subscribe<std_msgs::Bool>(
      "/file_player_start", 1,
      boost::bind(&ROSThread::FilePlayerStart, this, _1));
  stop_sub_ = nh_.subscribe<std_msgs::Bool>(
      "/file_player_stop", 1,
      boost::bind(&ROSThread::FilePlayerStop, this, _1));
  ...
  continental_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/continental/points", 10000);
  continentalobject_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(
      "/continentalobject/points", 10000);

  radarpolar_pub_ =
      nh_.advertise<sensor_msgs::Image>("/radar/polar", 10);  // giseop

  stereo_left_pub_ =
      nh_.advertise<sensor_msgs::Image>("/stereo/left/image_raw", 10);
  stereo_right_pub_ =
      nh_.advertise<sensor_msgs::Image>("/stereo/right/image_raw", 10);

  // stereo_left_info_pub_ =
  // nh_.advertise<sensor_msgs::CameraInfo>("/stereo/left/camera_info", 10);
  // stereo_right_info_pub_ =
  // nh_.advertise<sensor_msgs::CameraInfo>("/stereo/right/camera_info", 10);

  clock_pub_ = nh_.advertise<rosgraph_msgs::Clock>("/clock", 1);

  pose_gt_pub_ = nh_.advertise<geometry_msgs::PoseStamped>(
      "/groundtruth_continental/pose", 1000);
}
...
void ROSThread::Ready() {
  ...
  // double latitude, longitude, altitude, altitude_orthometric;
  double height, north_velocity, east_velocity, up_velocity, roll, pitch,
      azimuth;
  // string status;
  char status[17];
  int status_value;
  ...
  inspva_data_.clear();
  while (fscanf(fp, "%ld,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%s %d", &stamp,
                &latitude, &longitude, &height, &north_velocity, &east_velocity,
                &up_velocity, &roll, &pitch, &azimuth, status,
                &status_value) == 12) {
    // 17%19[^\n] %29[^\n]
    ...
    inspva_data.status =
        std::string(status) + " " + std::to_string(status_value);
  ...
  GetDirList(data_folder_path_ + "/Radar/Continentalobject",
             continentalobject_file_list_);
  GetDirList(data_folder_path_ + "/Radar/Navtech", radarpolar_file_list_);
  GetDirList(data_folder_path_ + "/Image/stereo_right",
             stereo_right_file_list_);
  ...
  // left_cinfo_ = boost::shared_ptr<camera_info_manager::CameraInfoManager>(new
  // camera_info_manager::CameraInfoManager(left_camera_nh_,"/stereo/left"));
  // right_cinfo_ =
  // boost::shared_ptr<camera_info_manager::CameraInfoManager>(new
  // camera_info_manager::CameraInfoManager(right_camera_nh_,"/stereo/right"));

  // string left_yaml_file_path = "file://" + data_folder_path_ +
  // "/calibration/left.yaml"; string right_yaml_file_path = "file://" +
  // data_folder_path_ + "/calibration/right.yaml";
  ...
  continental_thread_.thread_ =
      std::thread(&ROSThread::ContinentalThread, this);
  continentalobject_thread_.thread_ =
      std::thread(&ROSThread::ContinentalobjectThread, this);
  radarpolar_thread_.thread_ = std::thread(&ROSThread::RadarpolarThread, this);
  stereo_right_thread_.thread_ =
      std::thread(&ROSThread::StereorightThread, this);
  stereo_left_thread_.thread_ = std::thread(&ROSThread::StereoleftThread, this);

  // fp와 stamp는 함수 위쪽에서 이미 선언되어 있다고 가정함
  fp = fopen((data_folder_path_ + "/PR_GT/Continental_gt.txt").c_str(), "r");
  if (!fp) {
    std::cerr << "Failed to open groundtruth file: "
              << data_folder_path_ + "/PR_GT/Continental_gt.txt" << std::endl;
    return;
  }
  ...
  while (fscanf(fp, "%ld %lf %lf %lf %lf %lf %lf %lf\n", &stamp, &px, &py, &pz,
                &qx, &qy, &qz, &qw) == 8) {
    pose.header.stamp.fromNSec(stamp);
    pose.header.frame_id = "continental";
    ...
    pose_gt_data_[stamp] = pose;
  }

  fclose(fp);
  std::cout << "Pose GT data are loaded" << std::endl;

  // Thread 활성화 및 시작
  pose_gt_thread_.active_ = true;
  pose_gt_thread_.thread_ = std::thread(&ROSThread::PoseGTThread, this);
}
...
void ROSThread::DataStampThread() {
  ...
  for (auto iter = data_stamp_.begin(); iter != data_stamp_.end(); iter++) {
    ...
    while ((stamp > (initial_data_stamp_ + processed_stamp_)) &&
           (data_stamp_thread_.active_ == true)) {
    ...
    if (stamp == stop_region_iter->first) {
      if (stop_skip_flag_ == true) {
        ...
        iter =
            data_stamp_.find(stop_region_iter->second);  // find stop region end
        iter = prev(iter, 1);
    ...
    if (iter->second.compare("gps") == 0) {
      ...
      continentalobject_thread_.cv_.notify_all();
    } else if (iter->second.compare("navtech") == 0 &&
               radarpolar_active_ == true) {
      radarpolar_thread_.push(stamp);
      radarpolar_thread_.cv_.notify_all();
    } else if (iter->second.compare("stereo_right") == 0 &&
               stereo_right_active_ == true) {
      stereo_right_thread_.push(stamp);
      stereo_right_thread_.cv_.notify_all();
    } else if (iter->second.compare("stereo_left") == 0 &&
               stereo_left_active_ == true) {
      stereo_left_thread_.push(stamp);
      stereo_left_thread_.cv_.notify_all();
    } else if (iter->second == "pose_gt") {
      pose_gt_thread_.push(stamp);
      pose_gt_thread_.cv_.notify_all();
    }
    stamp_show_count_++;
    ...
    if (save_flag_ == true && process_flag_ == false) {
      bag_.open(data_folder_path_ + "/" + to_string(bag_idx_) + ".bag",
    ...
    while (!inspva_thread_.data_queue_.empty()) {
      ...
      if (inspva_data_.find(data) != inspva_data_.end()) {
        if (save_flag_ == true && process_flag_ == true) {
          ...
          bag_.write("/inspva", inspva_data_[data].header.stamp,
                     inspva_data_[data]);
        }
  ...
  while (1) {
    ...
    while (!imu_thread_.data_queue_.empty()) {
      ...
      if (imu_data_.find(data) != imu_data_.end()) {
        if (save_flag_ == true && process_flag_ == true) {
          std::lock_guard<std::mutex> lock(bag_mutex_);
          bag_.write("/imu/data_raw", imu_data_[data].header.stamp,
                     imu_data_[data]);
        }
...
void ROSThread::TimerCallback(const ros::TimerEvent &) {
  ...
  if (play_flag_ == true && pause_flag_ == false) {
    processed_stamp_ += static_cast<int64_t>(
        static_cast<double>(current_stamp - pre_timer_stamp_) * play_rate_);
  }
  ...
  while (1) {
    ...
    while (!aeva_thread_.data_queue_.empty()) {
      ...
      if (to_string(data) + ".bin" == aeva_next_.first) {
        ...
        if (save_flag_ == true && process_flag_ == true) {
          std::lock_guard<std::mutex> lock(bag_mutex_);
          bag_.write("/aeva/points", aeva_next_.second.header.stamp,
                     aeva_next_.second);
        }
        ...
        string current_file_name =
            data_folder_path_ + "/LiDAR/Aeva" + "/" + to_string(data) + ".bin";
        if (find(next(aeva_file_list_.begin(),
                      max(0, previous_file_index - search_bound_)),
                 aeva_file_list_.end(),
                 to_string(data) + ".bin") != aeva_file_list_.end()) {
          ifstream file;
          ...
          while (!file.eof()) {
            ...
            file.read(reinterpret_cast<char *>(&point.reflectivity),
                      sizeof(float));
            file.read(reinterpret_cast<char *>(&point.velocity), sizeof(float));
            file.read(reinterpret_cast<char *>(&point.time_offset_ns),
                      sizeof(int32_t));
            file.read(reinterpret_cast<char *>(&point.line_index),
                      sizeof(uint8_t));
      ...
      sensor_msgs::PointCloud2 publish_cloud;
      current_file_index =
          find(next(aeva_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               aeva_file_list_.end(), to_string(data) + ".bin") -
          aeva_file_list_.begin();
      if (find(next(aeva_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               aeva_file_list_.end(),
               aeva_file_list_[current_file_index + 1]) !=
          aeva_file_list_.end()) {
        string next_file_name = data_folder_path_ + "/LiDAR/Aeva" + "/" +
                                aeva_file_list_[current_file_index + 1];
        ...
        pcl::toROSMsg(cloud, publish_cloud);
        aeva_next_ =
            make_pair(aeva_file_list_[current_file_index + 1], publish_cloud);
      }
  ...
  while (1) {
    ...
    while (!continental_thread_.data_queue_.empty()) {
      ...
      if (to_string(data) + ".bin" == continental_next_.first) {
        ...
        if (save_flag_ == true && process_flag_ == true) {
          std::lock_guard<std::mutex> lock(bag_mutex_);
          bag_.write("/continental/points",
                     continental_next_.second.header.stamp,
                     continental_next_.second);
        }
        ...
        cloud.clear();
        sensor_msgs::PointCloud2 publish_cloud;
        string current_file_name = data_folder_path_ + "/Radar/Continental" +
                                   "/" + to_string(data) + ".bin";
        if (find(next(continental_file_list_.begin(),
                      max(0, previous_file_index - search_bound_)),
                 continental_file_list_.end(),
                 to_string(data) + ".bin") != continental_file_list_.end()) {
          ifstream file;
          file.open(current_file_name, ios::in | ios::binary);
          while (!file.eof()) {
            pc_type_c point;
            ...
            file.read(reinterpret_cast<char *>(&point.azimuth),
                      sizeof(point.azimuth));
            file.read(reinterpret_cast<char *>(&point.elevation),
                      sizeof(point.elevation));
            cloud.points.push_back(point);
          }
          file.close();
      ...
      sensor_msgs::PointCloud2 publish_cloud;
      current_file_index =
          find(next(continental_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               continental_file_list_.end(), to_string(data) + ".bin") -
          continental_file_list_.begin();
      if (find(next(continental_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               continental_file_list_.end(),
               continental_file_list_[current_file_index + 1]) !=
          continental_file_list_.end()) {
        string next_file_name = data_folder_path_ + "/Radar/Continental" + "/" +
                                continental_file_list_[current_file_index + 1];
        ...
        file.close();
        pcl::toROSMsg(cloud, publish_cloud);
        continental_next_ = make_pair(
            continental_file_list_[current_file_index + 1], publish_cloud);
      }
  ...
  while (1) {
    ...
    while (!continentalobject_thread_.data_queue_.empty()) {
      ...
      if (to_string(data) + ".bin" == continentalobject_next_.first) {
        ...
        if (save_flag_ == true && process_flag_ == true) {
          std::lock_guard<std::mutex> lock(bag_mutex_);
          bag_.write("/continentalobject/points",
                     continentalobject_next_.second.header.stamp,
                     continentalobject_next_.second);
        }
        ...
        cloud.clear();
        sensor_msgs::PointCloud2 publish_cloud;
        string current_file_name = data_folder_path_ +
                                   "/Radar/Continentalobject" + "/" +
                                   to_string(data) + ".bin";
        if (find(next(continentalobject_file_list_.begin(),
                      max(0, previous_file_index - search_bound_)),
                 continentalobject_file_list_.end(),
                 to_string(data) + ".bin") !=
            continentalobject_file_list_.end()) {
          ifstream file;
          file.open(current_file_name, ios::in | ios::binary);
      ...
      sensor_msgs::PointCloud2 publish_cloud;
      current_file_index =
          find(next(continentalobject_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               continentalobject_file_list_.end(), to_string(data) + ".bin") -
          continentalobject_file_list_.begin();
      if (find(next(continentalobject_file_list_.begin(),
                    max(0, previous_file_index - search_bound_)),
               continentalobject_file_list_.end(),
               continentalobject_file_list_[current_file_index + 1]) !=
          continentalobject_file_list_.end()) {
        string next_file_name =
            data_folder_path_ + "/Radar/Continentalobject" + "/" +
            continentalobject_file_list_[current_file_index + 1];
        ...
        file.close();
        pcl::toROSMsg(cloud, publish_cloud);
        continentalobject_next_ =
            make_pair(continentalobject_file_list_[current_file_index + 1],
                      publish_cloud);
      }
...
void ROSThread::RadarpolarThread() {
  ...
  while (1) {
    ...
    while (!radarpolar_thread_.data_queue_.empty()) {
      ...
      // publish
      if (to_string(data) + ".png" == radarpolar_next_.first &&
          !radarpolar_next_.second.empty()) {
        ...
        string current_radarpolar_name = data_folder_path_ + "/Radar/Navtech" +
                                         "/" + to_string(data) + ".png";

        cv::Mat radarpolar_image;
        radarpolar_image =
            imread(current_radarpolar_name, cv::IMREAD_GRAYSCALE);
      ...
      // load next image
      current_img_index =
          find(next(radarpolar_file_list_.begin(),
                    max(0, previous_img_index - search_bound_)),
               radarpolar_file_list_.end(), to_string(data) + ".png") -
          radarpolar_file_list_.begin();
      if (current_img_index < radarpolar_file_list_.size() - 2) {
        string next_radarpolar_name =
            data_folder_path_ + "/Radar/Navtech" + "/" +
            radarpolar_file_list_[current_img_index + 1];
        ...
        if (!radarpolar_image.empty()) {
          ...
          radarpolar_next_ = make_pair(
              radarpolar_file_list_[current_img_index + 1], radarpolar_image);
        }
      }
  ...
  while (1) {
    ...
    while (!stereo_left_thread_.data_queue_.empty()) {
      ...
      if (to_string(data) + ".png" == stereo_left_next_img_.first &&
          !stereo_left_next_img_.second.empty()) {
        cv_bridge::CvImage left_out_msg;
        left_out_msg.header.stamp.fromNSec(data);
        left_out_msg.header.frame_id = "stereo_left";
        left_out_msg.encoding =
            sensor_msgs::image_encodings::BGR8;  // Change to BGR8
        left_out_msg.image = stereo_left_next_img_.second;

        stereo_left_pub_.publish(left_out_msg.toImageMsg());
      } else {
        // Load left stereo image
        string current_stereo_left_name = data_folder_path_ +
                                          "/Image/stereo_left/" +
                                          to_string(data) + ".png";
        cv::Mat current_left_image = imread(
            current_stereo_left_name, cv::IMREAD_COLOR);  // Load as color image
      ...
      current_img_index =
          find(next(stereo_left_file_list_.begin(),
                    max(0, previous_img_index - search_bound_)),
               stereo_left_file_list_.end(), to_string(data) + ".png") -
          stereo_left_file_list_.begin();
      if (current_img_index < stereo_left_file_list_.size() - 1) {
        string next_stereo_left_name =
            data_folder_path_ + "/Image/stereo_left/" +
            stereo_left_file_list_[current_img_index + 1];
        cv::Mat next_left_image = imread(
            next_stereo_left_name, cv::IMREAD_COLOR);  // Load as color image
        if (!next_left_image.empty()) {
          stereo_left_next_img_ = make_pair(
              stereo_left_file_list_[current_img_index + 1], next_left_image);
        }
      }
      previous_img_index = current_img_index;
    }
  ...
  while (1) {
    ...
    while (!stereo_right_thread_.data_queue_.empty()) {
      ...
      if (to_string(data) + ".png" == stereo_right_next_img_.first &&
        ...
        string current_stereo_right_name = data_folder_path_ +
                                           "/Image/stereo_right/" +
                                           to_string(data) + ".png";
        cv::Mat current_right_image =
            imread(current_stereo_right_name,
                   cv::IMREAD_COLOR);  // Load as color image
      ...
      current_img_index =
          find(next(stereo_right_file_list_.begin(),
                    max(0, previous_img_index - search_bound_)),
               stereo_right_file_list_.end(), to_string(data) + ".png") -
          stereo_right_file_list_.begin();
      if (current_img_index < stereo_right_file_list_.size() - 1) {
        string next_stereo_right_name =
            data_folder_path_ + "/Image/stereo_right/" +
            stereo_right_file_list_[current_img_index + 1];
        cv::Mat next_right_image = imread(
            next_stereo_right_name, cv::IMREAD_COLOR);  // Load as color image
        if (!next_right_image.empty()) {
          stereo_right_next_img_ = make_pair(
              stereo_right_file_list_[current_img_index + 1], next_right_image);
        }
      }
      previous_img_index = current_img_index;
    }
...
int ROSThread::GetDirList(string dir, vector<string> &files) {
  ...
  if (n < 0)
    ...
    while (n--) {
      if (string(namelist[n]->d_name) != "." &&
          string(namelist[n]->d_name) != "..") {
...
void ROSThread::ResetProcessStamp(int position) {
  if (position > 0 && position < 10000) {
    processed_stamp_ = static_cast<int64_t>(
        static_cast<float>(last_data_stamp_ - initial_data_stamp_) *
        static_cast<float>(position) / static_cast<float>(10000));
    reset_process_stamp_flag_ = true;
  }
}

void ROSThread::PoseGTThread() {
  while (1) {
    std::unique_lock<std::mutex> ul(pose_gt_thread_.mutex_);
    pose_gt_thread_.cv_.wait(ul);
    if (!pose_gt_thread_.active_) return;
    ul.unlock();

    while (!pose_gt_thread_.data_queue_.empty()) {
      auto stamp = pose_gt_thread_.pop();

      if (pose_gt_data_.find(stamp) != pose_gt_data_.end()) {
        pose_gt_pub_.publish(pose_gt_data_[stamp]);
      }
    }
